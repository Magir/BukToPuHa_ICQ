...
define('administrator_id',27222954); // Change administrator_id to your Telegram ID
define('bote_id','001.0301874641.3763326743:752124334'); // Change this to your bot ID, get bot ID from BotFather (https://core.telegram.org/bots)
global $bot;
global $players;
global $allplayers;
global $top;
...
function message($userid,$text){
	...
	if (isset($players[$userid])){
		...
		$bot->send($userid,'Наберите /on чтобы войти в игру.'); // "Send /start command to enter the game."
	}
}
...
function start($userid,$first_name,$last_name){
	...
	if (!isset($players[$userid])){
		if (!isset($banlist[$userid])){
			...
			$bot->send($userid,'Вы вошли в игру. Читайте вопросы и пишите ответы! Чтобы выйти из игры наберите /off'); // "You've entered game. Read questions and write answers. To leave game send /stop"
...
function stop($userid){
	...
	if (isset($players[$userid])){
		$bot->send($userid,"Вы вышли из игры. Чтобы вернуться в игру наберите /on"); // "You've left game. To enter again write /start "
